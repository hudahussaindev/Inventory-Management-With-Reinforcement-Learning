...
def main():
    """
    Main entry point:
        - Train and evaluate Q-Learning on the discrete-action environment.
        - Train and evaluate PPO on the continuous-action environment.
    """
    #Q-LEARNING (DISCRETE ACTIONS)
    print("Setting up Q-Learning evaluation...")
    env_disc = ExtendedInventoryEnv(discrete_actions=True, seed=42)
    evaluator_disc = InventoryEvaluator(env_disc)
    
    # Train Q-Learning agent
    print("Training Q-Learning agent...")
    from rl_inventory.scripts.q_learning_demo import train_agent as train_q_agent
    q_agent, q_disc = train_q_agent(num_episodes=200)
    
    # Evaluate Q-Learning
    print("\nEvaluating Q-Learning...")
    q_metrics = evaluator_disc.evaluate_multiple(q_agent, q_disc, num_episodes=10)
    evaluator_disc.print_report(q_metrics, "Q-Learning")
    ...
    ppo_agent, _ = train_ppo_agent(num_timesteps=200000)
    
    # Evaluate PPO (no discretizer; evaluate_episode will call agent.predict)
    print("\nEvaluating PPO...")
    ppo_metrics = evaluator_cont.evaluate_multiple(
        ppo_agent,
        discretizer=None,
        num_episodes=10,
    )
    evaluator_cont.print_report(ppo_metrics, "PPO")
